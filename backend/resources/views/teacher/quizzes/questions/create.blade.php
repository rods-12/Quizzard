...
<div style="margin-bottom:28px;">
    ...
    <h1 style="font-size:24px; font-weight:700; color:var(--text); letter-spacing:-0.03em; line-height:1.2; margin-bottom:6px;">Add Question</h1>
    <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
        <p style="font-size:13px; color:var(--text-2);">{{ $quiz->title }}</p>
        <span style="color:var(--text-3); font-size:12px;">·</span>
        <span class="badge badge-sky" style="font-size:10px; text-transform:capitalize;">{{ str_replace('_', ' ', $type) }}</span>
    </div>
</div>
...
<div style="max-width:680px;">
    <div class="card" style="overflow:hidden;">
        ...
        <div style="padding:18px 22px; border-bottom:1px solid var(--border);">
            <p style="font-size:10px; font-weight:700; letter-spacing:0.1em; text-transform:uppercase; color:var(--accent); margin-bottom:4px;">New Question</p>
            <h2 style="font-size:14px; font-weight:700; color:var(--text); margin-bottom:2px;">Question Details</h2>
            <p style="font-size:12px; color:var(--text-2);">Fill in the question, set the point value, and provide the correct answer.</p>
        </div>
            ...
            <form action="{{ route('teacher.quizzes.questions.store', $quiz->id) }}" method="POST">
                ...
                <div style="margin-bottom:18px;">
                    ...
                    <textarea name="question_text" rows="3" required
                              ...
                              onblur="this.style.borderColor='var(--border-md)'">{{ old('question_text') }}</textarea>
                </div>
                ...
                <div style="margin-bottom:24px;">
                    ...
                    <div style="display:flex; gap:12px;">
                        <label style="display:flex; align-items:center; gap:8px; cursor:pointer; font-size:13px; color:var(--text); background:var(--surface-3); border:1px solid var(--border-md); border-radius:var(--radius-sm); padding:9px 16px; transition:border-color 0.15s;"
                            ...
                            <span style="font-weight:600;">True</span>
                        </label>
                        <label style="display:flex; align-items:center; gap:8px; cursor:pointer; font-size:13px; color:var(--text); background:var(--surface-3); border:1px solid var(--border-md); border-radius:var(--radius-sm); padding:9px 16px; transition:border-color 0.15s;">
                            <input type="radio" name="correct_tf" value="false"
                                   {{ old('correct_tf') === 'false' ? 'checked' : '' }}
                                   style="accent-color:var(--accent); width:14px; height:14px; cursor:pointer;">
                            <span style="font-weight:600;">False</span>
                        </label>
                    </div>
                </div>
                @endif
