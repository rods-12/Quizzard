...
@section('content')
    ...
    <div style="margin-bottom:28px;">
        ...
        <h1 style="font-size:24px; font-weight:700; color:var(--text); letter-spacing:-0.03em; line-height:1.2; margin-bottom:6px;">Classes</h1>
        <p style="font-size:13px; color:var(--text-2);">View a summary of your classes, including student count, quiz assignments, and average score.</p>
    </div>
    ...
    <div class="card" style="overflow:hidden;">
        ...
        <div style="display:flex; align-items:center; justify-content:space-between; gap:16px; padding:18px 22px; border-bottom:1px solid var(--border);">
            <div>
                ...
                <h2 style="font-size:14px; font-weight:700; color:var(--text); margin-bottom:2px;">Class Reports</h2>
                <p style="font-size:12px; color:var(--text-2);">This table shows all classes created by you.</p>
            </div>
            <span class="badge badge-green" style="flex-shrink:0;">Class &amp; Activity Overview</span>
        </div>
        ...
        @if($classes->isEmpty())
            ...
            <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; padding:64px 24px; text-align:center;">
                ...
                <p style="font-size:14px; font-weight:700; color:var(--text); margin-bottom:6px;">No classes found</p>
                <p style="font-size:13px; color:var(--text-2); max-width:280px;">There are currently no class records available in this report.</p>
            </div>
                ...
                <table id="classesTable" style="width:100%; border-collapse:collapse;">
                    <thead>
                        <tr style="border-bottom:1px solid var(--border);">
                            <th onclick="sortTable('classesTable', 0)"
                                ...
                                Class Name <span class="sort-icon" style="opacity:0.5;">↕</span>
                            </th>
                            <th onclick="sortTable('classesTable', 1)"
                                ...
                                onmouseenter="this.style.color='var(--text-2)'" onmouseleave="this.style.color='var(--text-3)'">
                                Class Code <span class="sort-icon" style="opacity:0.5;">↕</span>
                            </th>
                            <th onclick="sortTable('classesTable', 2)"
                                style="padding:12px 16px; text-align:left; font-size:10px; font-weight:700; letter-spacing:0.1em; text-transform:uppercase; color:var(--text-3); cursor:pointer; white-space:nowrap; user-select:none;"
                                onmouseenter="this.style.color='var(--text-2)'" onmouseleave="this.style.color='var(--text-3)'">
                                Students <span class="sort-icon" style="opacity:0.5;">↕</span>
                            </th>
                            <th onclick="sortTable('classesTable', 3)"
                                style="padding:12px 16px; text-align:left; font-size:10px; font-weight:700; letter-spacing:0.1em; text-transform:uppercase; color:var(--text-3); cursor:pointer; white-space:nowrap; user-select:none;"
                                onmouseenter="this.style.color='var(--text-2)'" onmouseleave="this.style.color='var(--text-3)'">
                                Quizzes <span class="sort-icon" style="opacity:0.5;">↕</span>
                            </th>
                        ...
                        @foreach($classes as $class)
                            <tr class="divider"
                                ...
                                <td style="padding:14px 16px; white-space:nowrap;">
                                    <span class="chip num">{{ $class->class_code }}</span>
                                </td>
                                <td style="padding:14px 16px; white-space:nowrap;">
                                    <span class="num" style="font-size:13px; color:var(--text-2);">{{ $class->students_count }}</span>
                                </td>
                                <td style="padding:14px 16px; white-space:nowrap;">
                                    <span class="num" style="font-size:13px; color:var(--text-2);">{{ $class->quizzes_count }}</span>
                                </td>
